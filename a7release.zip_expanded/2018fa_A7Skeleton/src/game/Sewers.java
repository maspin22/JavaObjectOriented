...
/** An instance is a Sewers through which a sewer diver can move.
 * It's a grid of Tile objects with a weighted graph of all non-floor tiles.
 *
 * There is an entrance to the sewer system and a target location
...
public class Sewers {
    ...
    /** Return a new random sewer system of the given size where there are no
    ...
    public static Sewers digExploreSewer(int rows, int cols, Random rand) {
        ...
        Sewers sewers= new Sewers(rows, cols, rand, () -> 1, () -> 0, Tile.Type.RING);
        while (sewers.minPathLengthToTarget(sewers.getEntrance()) < minRingDist) {
            sewers= new Sewers(rows, cols, rand, () -> 1, () -> 0, Tile.Type.RING);
        }
        return sewers;
    }
    ...
    /** Return a new random sewer system with random coins and edge weights.
     ...
     * @param rand A source of randomness to use for the sewer-system generation.
     */
    public static Sewers digGetOutSewer(int rows, int cols, int currentRow, int currentCol, Random rand) {
        Supplier<Integer> edgeWeightGen= () -> rand.nextInt(MAX_EDGE_WEIGHT) + 1;
        Supplier<Integer> coinGen= () -> Sewers.generateCoinValue(rand);
        Sewers potentialCavern= new Sewers(rows, cols, rand, edgeWeightGen, coinGen, Tile.Type.ENTRANCE);
        while (potentialCavern.getTileAt(currentRow, currentCol).getType() != Tile.Type.FLOOR) {
            potentialCavern= new Sewers(rows, cols, rand, edgeWeightGen, coinGen, Tile.Type.ENTRANCE);
        }
     ...
     * This uses {@code edgeWeightGenerator} and {@code coinGenerator} to generate edge weights and coin values.
     * Precondition: {@code targetType} must be either {@code Tile.Type.RING} or {@code Tile.Type.ENTRANCE}.
     */
    ...
    private Set<Node> generateGraph(Random rand,
        ...
        while (!frontier.isEmpty()) {
            ...
            if (nExits > 0) {
                ...
                .peek(openPoints::add)
                .map((q) -> new Node(new Tile(q.row, q.col, coinGenerator.get(), Tile.Type.FLOOR), cols))
                .peek(frontier::add)
                .forEach(nodes::add);
            }
        }
    ...
    /** Return a randomly chosen entrance to the sewer system (the only non-wall tile
    ...
    /** Return the number of open floor tiles in this sewer system
     ...
     * only the minimum distance between the given node and Node target (a field)
     * for this graph (no path).
     * Precondition: {@code start} must be a node of the graph.
     */
    int minPathLengthToTarget(Node start) {
        Map<Long, Integer> pathDists= new HashMap<>(); // the keys are NodeIds.
        InternalMinHeap<Node> F= new InternalMinHeap<>();

        pathDists.put(start.getId(), 0);
        F.add(start, 0);
        /** Invariant: as in class notes on Dijkstra's shortest path algorithm.
         * AND: pathDists contains an entry for each node in the settled and
         * frontier (F) sets; its value is the current shortest distance
         * from the start node that that node.
         */
        while (!F.isEmpty()) {
            Node f= F.poll();
            if (f.equals(target)) {
                return pathDists.get(f.getId());
            }

            int fDist= pathDists.get(f.getId());

            for (Edge e : f.getExits()) {
                Node w= e.getOther(f);
                int newWDist= fDist + e.length();
                Integer oldWDist= pathDists.get(w.getId());
                if (oldWDist == null) {
                    pathDists.put(w.getId(), newWDist);
                    F.add(w, newWDist);
                } else if (newWDist < oldWDist) {
                    pathDists.put(w.getId(), newWDist);
    ...
    /** Serialize this sewer system to a list of strings which can be written out to a file.
     * The list of strings can be converted back into a {@code Sewers} using {@code deserialize()}.
     */
